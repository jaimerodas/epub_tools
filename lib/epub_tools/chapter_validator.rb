...
module EpubTools
  ...
  class ChapterValidator
    ...
    # Half-chapters (e.g. chapter_5_5.xhtml) and lettered side chapters (e.g. chapter_5a.xhtml) are
    ...
    def validate
      ...
      log "Chapter sequence is complete: #{nums.first.floor} to #{nums.last.floor}."
    end
    ...
    def extract_chapter_number(basename)
      if (m = basename.match(/_(\d+)(?:_5|[a-z]+)\z/))
        m[1].to_i + 0.5
    ...
    def check_sequence_completeness(sorted)
      ...
      return if integers.empty? # e.g. appending only side chapters
